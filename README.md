...
| `durationFilterEnabled` | 再生時間フィルタの有効/無効 | 無効 | — |
| `durationMinMinutes` | フィルタの最短時間（分。0 で下限なし） | 0 | 0–600 |
| `durationMaxMinutes` | フィルタの最長時間（分。0 で上限なし・以内） | 0 | 0–600 |
| `hideSkippedEnabled` | スキップ済み動画をリセットまで再表示しないか | 無効 | — |
...
entrypoints/
  ...
  background.ts        # MV3 service worker（キーボードコマンドを対象タブへ仲介）
...
4. 「次へ」でスキップした動画は `tubeflow-dismissed`（storage.local）に記録され、リセットまでホームから除外される。
...
- **E2E（決定的）**: `.output/chrome-mv3` を実際に読み込み、`www.youtube.com` をローカル静的フィクスチャでスタブ。先頭 N 件表示・棚の非表示・ページ送り・カード個別ボタン・中央寄せ/幅・設定反映・スキップ済み非表示/リセット・利用制限オーバーレイ・エンドスクリーン抑制を検証。`pnpm build` を先に実行。オフラインで安定。
