...
**利用制限の内部動作**: 視聴秒数は端末ローカルの `chrome.storage.local`（キー `tubeflow-usage`、`{ date, seconds }`）に保存し、日付が変われば 0 リセット。判定ロジックは `lib/restrictions.ts`（純粋関数）、計測・遮断の実行は `lib/content/blocker.ts`。ブロック中は全画面オーバーレイ `#tf-block-overlay` を表示し、再生中の動画も停止します。
