...
- **「次へ」回数の可視化**: 「次へ」を押した回数を**その日単位で記録**し、右下ミニ UI にバッジ（`本日N回`、回数が増えると色が変化）、ポップアップ/オプションに数値で表示します。回数は `chrome.storage.local` に保存され**再読み込み・タブを開き直しても残り**、毎日 0 時にリセット。「今日もう何回スキップしたか」を自覚するための機能です。
...
|------|-----------|--------------------------|------|
| 次へ（ページ送り） | Alt+J | 右下ミニ UI「次へ」 | 表示枚数ぶんまとめて送る。回数無制限。本日の押下回数をバッジ表示 |
...
**使用量の内部動作**: 視聴秒数と「次へ」回数は端末ローカルの `chrome.storage.local`（キー `tubeflow-usage`、`{ date, seconds, skips }`）に保存し、日付が変われば 0 リセット。判定ロジックは `lib/restrictions.ts`（純粋関数）、計測・遮断の実行は `lib/content/blocker.ts`。ブロック中は全画面オーバーレイ `#tf-block-overlay` を表示し、再生中の動画も停止します。
