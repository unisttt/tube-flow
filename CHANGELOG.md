...
### 変更（Changed）
- **スキップ回数の閾値＋タブクローズを廃止**（`skipCloseThreshold`・`request-exit`・背景のタブクローズを削除）。更新/開き直しで無意味なため。代わりに**「次へ」は回数無制限**にし、**押した回数を日単位で記録・可視化**する方式へ変更。回数は `chrome.storage.local`（`tubeflow-usage.skips`）に保存され再読み込みでも残り、毎日 0 時にリセット。右下ミニ UI にバッジ（`本日N回`）、ポップアップ/オプションに数値表示。
